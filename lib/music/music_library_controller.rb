...
class MusicLibraryController
    ...
    def list_artists
        array = Artist.all.uniq
        array.sort_by { |s| s.name }.each_with_index { |obj, i| puts "#{i + 1}. #{obj.name}" }
    end

    def list_genres
        array = Genre.all.uniq
        array.sort_by { |s| s.name }.each_with_index { |obj, i| puts "#{i + 1}. #{obj.name}" }
    end 
    ...
    def list_songs_by_artist
        ...
        text = gets.strip
        artist_instance = Artist.find_by_name(text)
    ...
    def list_songs_by_genre
        ...
        text = gets.strip
        genre_instance = Genre.find_by_name(text)
